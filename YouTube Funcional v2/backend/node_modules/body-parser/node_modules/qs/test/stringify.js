...
test('stringify()', function (t) {
    t.test('stringifies a querystring object', function (st) {
        ...
        st.equal(qs.stringify({ a: 'ÃÂ¢ÃÂÃÂ¬' }), 'a=%E2%82%AC');
        st.equal(qs.stringify({ a: 'ÃÂ®ÃÂÃÂ' }), 'a=%EE%80%80');
        st.equal(qs.stringify({ a: 'ÃÂÃÂ' }), 'a=%D7%90');
        st.equal(qs.stringify({ a: 'ÃÂ°ÃÂÃÂÃÂ·' }), 'a=%F0%90%90%B7');
        st.end();
    });
    ...
    t.test('can stringify with custom encoding', function (st) {
        st.equal(qs.stringify({ ÃÂ§ÃÂÃÂ: 'ÃÂ¥ÃÂ¤ÃÂ§ÃÂ©ÃÂÃÂªÃÂ¥ÃÂºÃÂ', '': '' }, {
    ...
    t.test('respects a charset of iso-8859-1', function (st) {
        st.equal(qs.stringify({ ÃÂÃÂ¦: 'ÃÂÃÂ¦' }, { charset: 'iso-8859-1' }), '%E6=%E6');
        st.end();
    });

    t.test('encodes unrepresentable chars as numeric entities in iso-8859-1 mode', function (st) {
        st.equal(qs.stringify({ a: 'ÃÂ¢ÃÂÃÂº' }, { charset: 'iso-8859-1' }), 'a=%26%239786%3B');
        st.end();
    });

    t.test('respects an explicit charset of utf-8 (the default)', function (st) {
        st.equal(qs.stringify({ a: 'ÃÂÃÂ¦' }, { charset: 'utf-8' }), 'a=%C3%A6');
        st.end();
    });

    t.test('`charsetSentinel` option', function (st) {
        st.equal(
            qs.stringify({ a: 'ÃÂÃÂ¦' }, { charsetSentinel: true, charset: 'utf-8' }),
            'utf8=%E2%9C%93&a=%C3%A6',
            'adds the right sentinel when instructed to and the charset is utf-8'
        );

        st.equal(
            qs.stringify({ a: 'ÃÂÃÂ¦' }, { charsetSentinel: true, charset: 'iso-8859-1' }),
