...
${u}`)),l=[];if(typeof o!="string")return;JSON.parse(o).items.forEach(u=>{u&&l.push(new E(u))}),this.fetched_tracks.set(`${r}`,l),s("Success")}));return await Promise.allSettled(t),this}page(e){if(!e)throw new Error("Page number is not provided");if(!this.fetched_tracks.has(`${e}`))throw new Error("Given Page number is invalid");return this.fetched_tracks.get(`${e}`)}get total_pages(){return this.fetched_tracks.size}get total_tracks(){if(this.search)return this.tracksCount;let e=this.total_pages;return(e-1)*100+this.fetched_tracks.get(`${e}`).length}async all_tracks(){await this.fetch();let e=[];for(let t of this.fetched_tracks.values())e.push(...t);return e}toJSON(){return{name:this.name,id:this.id,type:this.type,url:this.url,thumbnail:this.thumbnail,artists:this.artists,copyrights:this.copyrights,release_date:this.release_date,release_date_precision:this.release_date_precision,tracksCount:this.tracksCount}}};a(z,"SpotifyAlbum");var J=require("fs"),d;(0,J.existsSync)(".data/spotify.data")&&(d=JSON.parse((0,J.readFileSync)(".data/spotify.data","utf-8")),d.file=!0);var yt=/^((https:)?\/\/)?open\.spotify\.com\/(?:intl\-.{2}\/)?(track|album|playlist)\//;async function Ce(i){if(!d)throw new Error(`Spotify Data is missing
Did you forgot to do authorization ?`);let e=i.trim();if(!e.match(yt))throw new Error("This is not a Spotify URL");if(e.indexOf("track/")!==-1){let t=e.split("track/")[1].split("&")[0].split("?")[0],r=await h(`https://api.spotify.com/v1/tracks/${t}?market=${d.market}`,{headers:{Authorization:`${d.token_type} ${d.access_token}`}}).catch(n=>n);if(r instanceof Error)throw r;let s=JSON.parse(r);if(s.error)throw new Error(`Got ${s.error.status} from the spotify request: ${s.error.message}`);return new E(s)}else if(e.indexOf("album/")!==-1){let t=i.split("album/")[1].split("&")[0].split("?")[0],r=await h(`https://api.spotify.com/v1/albums/${t}?market=${d.market}`,{headers:{Authorization:`${d.token_type} ${d.access_token}`}}).catch(n=>n);if(r instanceof Error)throw r;let s=JSON.parse(r);if(s.error)throw new Error(`Got ${s.error.status} from the spotify request: ${s.error.message}`);return new z(s,d,!1)}else if(e.indexOf("playlist/")!==-1){let t=i.split("playlist/")[1].split("&")[0].split("?")[0],r=await h(`https://api.spotify.com/v1/playlists/${t}?market=${d.market}`,{headers:{Authorization:`${d.token_type} ${d.access_token}`}}).catch(n=>n);if(r instanceof Error)throw r;let s=JSON.parse(r);if(s.error)throw new Error(`Got ${s.error.status} from the spotify request: ${s.error.message}`);return new N(s,d,!1)}else throw new Error("URL is out of scope for play-dl.")}a(Ce,"spotify");function ge(i){let e=i.trim();return e.startsWith("https")?e.match(yt)?e.indexOf("track/")!==-1?"track":e.indexOf("album/")!==-1?"album":e.indexOf("playlist/")!==-1?"playlist":!1:!1:"search"}a(ge,"sp_validate");async function bt(i,e){let t=await h("https://accounts.spotify.com/api/token",{headers:{Authorization:`Basic ${Buffer.from(`${i.client_id}:${i.client_secret}`).toString("base64")}`,"Content-Type":"application/x-www-form-urlencoded"},body:`grant_type=authorization_code&code=${i.authorization_code}&redirect_uri=${encodeURI(i.redirect_url)}`,method:"POST"}).catch(s=>s);if(t instanceof Error)throw t;let r=JSON.parse(t);return d={client_id:i.client_id,client_secret:i.client_secret,redirect_url:i.redirect_url,access_token:r.access_token,refresh_token:r.refresh_token,expires_in:Number(r.expires_in),expiry:Date.now()+(r.expires_in-1)*1e3,token_type:r.token_type,market:i.market},e?(0,J.writeFileSync)(".data/spotify.data",JSON.stringify(d,void 0,4)):(console.log(`Client ID : ${d.client_id}`),console.log(`Client Secret : ${d.client_secret}`),console.log(`Refresh Token : ${d.refresh_token}`),console.log(`Market : ${d.market}`),console.log(`
Paste above info in setToken function.`)),!0}a(bt,"SpotifyAuthorize");function Oe(){return Date.now()>=d.expiry}a(Oe,"is_expired");async function gt(i,e,t=10){let r=[];if(!d)throw new Error(`Spotify Data is missing
Did you forget to do authorization ?`);if(i.length===0)throw new Error("Pass some query to search.");if(t>50||t<0)throw new Error("You crossed limit range of Spotify [ 0 - 50 ]");let s=await h(`https://api.spotify.com/v1/search?type=${e}&q=${i}&limit=${t}&market=${d.market}`,{headers:{Authorization:`${d.token_type} ${d.access_token}`}}).catch(o=>o);if(s instanceof Error)throw s;let n=JSON.parse(s);return e==="track"?n.tracks.items.forEach(o=>{r.push(new E(o))}):e==="album"?n.albums.items.forEach(o=>{r.push(new z(o,d,!0))}):e==="playlist"&&n.playlists.items.forEach(o=>{r.push(new N(o,d,!0))}),r}a(gt,"sp_search");async function we(){let i=await h("https://accounts.spotify.com/api/token",{headers:{Authorization:`Basic ${Buffer.from(`${d.client_id}:${d.client_secret}`).toString("base64")}`,"Content-Type":"application/x-www-form-urlencoded"},body:`grant_type=refresh_token&refresh_token=${d.refresh_token}`,method:"POST"}).catch(t=>t);if(i instanceof Error)return!1;let e=JSON.parse(i);return d.access_token=e.access_token,d.expires_in=Number(e.expires_in),d.expiry=Date.now()+(e.expires_in-1)*1e3,d.token_type=e.token_type,d.file&&(0,J.writeFileSync)(".data/spotify.data",JSON.stringify(d,void 0,4)),!0}a(we,"refreshToken");async function wt(i){d=i,d.file=!1,await we()}a(wt,"setSpotifyToken");var _e=require("fs");var _t=require("stream");var T=class{constructor(e){this.name=e.title,this.id=e.id,this.url=e.uri,this.permalink=e.permalink_url,this.fetched=!0,this.type="track",this.durationInSec=Math.round(Number(e.duration)/1e3),this.durationInMs=Number(e.duration),e.publisher_metadata?this.publisher={name:e.publisher_metadata.publisher,id:e.publisher_metadata.id,artist:e.publisher_metadata.artist,contains_music:Boolean(e.publisher_metadata.contains_music)||!1,writer_composer:e.publisher_metadata.writer_composer}:this.publisher=null,this.formats=e.media.transcodings,this.user={name:e.user.username,id:e.user.id,type:"user",url:e.user.permalink_url,verified:Boolean(e.user.verified)||!1,description:e.user.description,first_name:e.user.first_name,full_name:e.user.full_name,last_name:e.user.last_name,thumbnail:e.user.avatar_url},this.thumbnail=e.artwork_url}toJSON(){return{name:this.name,id:this.id,url:this.url,permalink:this.permalink,fetched:this.fetched,durationInMs:this.durationInMs,durationInSec:this.durationInSec,publisher:this.publisher,formats:this.formats,thumbnail:this.thumbnail,user:this.user}}};a(T,"SoundCloudTrack");var P=class{constructor(e,t){this.name=e.title,this.id=e.id,this.url=e.uri,this.client_id=t,this.type="playlist",this.sub_type=e.set_type,this.durationInSec=Math.round(Number(e.duration)/1e3),this.durationInMs=Number(e.duration),this.user={name:e.user.username,id:e.user.id,type:"user",url:e.user.permalink_url,verified:Boolean(e.user.verified)||!1,description:e.user.description,first_name:e.user.first_name,full_name:e.user.full_name,last_name:e.user.last_name,thumbnail:e.user.avatar_url},this.tracksCount=e.track_count;let r=[];e.tracks.forEach(s=>{s.title?r.push(new T(s)):r.push({id:s.id,fetched:!1,type:"track"})}),this.tracks=r}async fetch(){let e=[];for(let t=0;t<this.tracks.length;t++)this.tracks[t].fetched||e.push(new Promise(async r=>{let s=t,n=await h(`https://api-v2.soundcloud.com/tracks/${this.tracks[t].id}?client_id=${this.client_id}`);this.tracks[s]=new T(JSON.parse(n)),r("")}));return await Promise.allSettled(e),this}get total_tracks(){let e=0;return this.tracks.forEach(t=>{if(t instanceof T)e++;else return}),e}async all_tracks(){return await this.fetch(),this.tracks}toJSON(){return{name:this.name,id:this.id,sub_type:this.sub_type,url:this.url,durationInMs:this.durationInMs,durationInSec:this.durationInSec,tracksCount:this.tracksCount,user:this.user,tracks:this.tracks}}};a(P,"SoundCloudPlaylist");var L=class{constructor(e,t="arbitrary"){this.stream=new _t.Readable({highWaterMark:5*1e3*1e3,read(){}}),this.type=t,this.url=e,this.downloaded_time=0,this.request=null,this.downloaded_segments=0,this.time=[],this.timer=new $(()=>{this.timer.reuse(),this.start()},280),this.segment_urls=[],this.stream.on("close",()=>{this.cleanup()}),this.start()}async parser(){let e=await h(this.url).catch(r=>r);if(e instanceof Error)throw e;e.split(`
`).forEach(r=>{r.startsWith("#EXTINF:")?this.time.push(parseFloat(r.replace("#EXTINF:",""))):r.startsWith("https")&&this.segment_urls.push(r)})}async start(){if(this.stream.destroyed){this.cleanup();return}this.time=[],this.segment_urls=[],this.downloaded_time=0,await this.parser(),this.segment_urls.splice(0,this.downloaded_segments),this.loop()}async loop(){if(this.stream.destroyed){this.cleanup();return}if(this.time.length===0||this.segment_urls.length===0){this.cleanup(),this.stream.push(null);return}this.downloaded_time+=this.time.shift(),this.downloaded_segments++;let e=await _(this.segment_urls.shift()).catch(t=>t);if(e instanceof Error){this.stream.emit("error",e),this.cleanup();return}this.request=e,e.on("data",t=>{this.stream.push(t)}),e.on("end",()=>{this.downloaded_time>=300||this.loop()}),e.once("error",t=>{this.stream.emit("error",t)})}cleanup(){this.timer.destroy(),this.request?.destroy(),this.url="",this.downloaded_time=0,this.downloaded_segments=0,this.request=null,this.time=[],this.segment_urls=[]}pause(){this.timer.pause()}resume(){this.timer.resume()}};a(L,"SoundCloudStream");var C;(0,_e.existsSync)(".data/soundcloud.data")&&(C=JSON.parse((0,_e.readFileSync)(".data/soundcloud.data","utf-8")));var kt=/^(?:(https?):\/\/)?(?:(?:www|m)\.)?(api\.soundcloud\.com|soundcloud\.com|snd\.sc)\/(.*)$/;async function ke(i){if(!C)throw new Error(`SoundCloud Data is missing
Did you forget to do authorization ?`);let e=i.trim();if(!e.match(kt))throw new Error("This is not a SoundCloud URL");let t=await h(`https://api-v2.soundcloud.com/resolve?url=${e}&client_id=${C.client_id}`).catch(s=>s);if(t instanceof Error)throw t;let r=JSON.parse(t);if(r.kind!=="track"&&r.kind!=="playlist")throw new Error("This url is out of scope for play-dl.");return r.kind==="track"?new T(r):new P(r,C.client_id)}a(ke,"soundcloud");async function vt(i,e,t=10){let r=await h(`https://api-v2.soundcloud.com/search/${e}?q=${i}&client_id=${C.client_id}&limit=${t}`),s=[];return JSON.parse(r).collection.forEach(o=>{e==="tracks"?s.push(new T(o)):s.push(new P(o,C.client_id))}),s}a(vt,"so_search");async function St(i,e){let t=await ke(i);if(t instanceof P)throw new Error("Streams can't be created from playlist urls");let r=xt(t.formats);typeof e!="number"?e=r.length-1:e<=0?e=0:e>=r.length&&(e=r.length-1);let s=r[e].url+"?client_id="+C.client_id,n=JSON.parse(await h(s)),o=r[e].format.mime_type.startsWith("audio/ogg")?"ogg/opus":"arbitrary";return new L(n.url,o)}a(St,"stream");async function $e(){let i=await h("https://soundcloud.com/",{headers:{}}).catch(s=>s);if(i instanceof Error)throw new Error("Failed to get response from soundcloud.com: "+i.message);let e=i.split('<script crossorigin src="'),t=[];return e.forEach(s=>{s.startsWith("https")&&t.push(s.split('"')[0])}),(await h(t[t.length-1])).split(',client_id:"')[1].split('"')[0]}a($e,"getFreeClientID");async function Et(i,e){let t=xt(i.formats);typeof e!="number"?e=t.length-1:e<=0?e=0:e>=t.length&&(e=t.length-1);let r=t[e].url+"?client_id="+C.client_id,s=JSON.parse(await h(r)),n=t[e].format.mime_type.startsWith("audio/ogg")?"ogg/opus":"arbitrary";return new L(s.url,n)}a(Et,"stream_from_info");async function Tt(i){return!(await h(`https://api-v2.soundcloud.com/search?client_id=${i}&q=Rick+Roll&limit=0`).catch(t=>t)instanceof Error)}a(Tt,"check_id");async function ve(i){let e=i.trim();if(!e.startsWith("https"))return"search";if(!e.match(kt))return!1;let t=await h(`https://api-v2.soundcloud.com/resolve?url=${e}&client_id=${C.client_id}`).catch(s=>s);if(t instanceof Error)return!1;let r=JSON.parse(t);return r.kind==="track"?"track":r.kind==="playlist"?"playlist":!1}a(ve,"so_validate");function xt(i){let e=[];return i.forEach(t=>{t.format.protocol==="hls"&&e.push(t)}),e}a(xt,"parseHlsFormats");function Dt(i){C=i}a(Dt,"setSoundCloudToken");var It=require("url");var g=class{constructor(e,t){this.id=e.id,this.title=e.title,this.shortTitle=e.title_short,this.url=e.link,this.durationInSec=e.duration,this.rank=e.rank,this.explicit=e.explicit_lyrics,this.previewURL=e.preview,this.artist=new M(e.artist),this.album=new Pe(e.album),this.type="track",this.partial=t,t||(this.trackPosition=e.track_position,this.diskNumber=e.disk_number,this.releaseDate=new Date(e.release_date),this.bpm=e.bpm,this.gain=e.gain,this.contributors=[],e.contributors.forEach(r=>{this.contributors?.push(new M(r))}))}async fetch(){if(!this.partial)return this;let e=await h(`https://api.deezer.com/track/${this.id}/`).catch(r=>r);if(e instanceof Error)throw e;let t=JSON.parse(e);return this.partial=!1,this.trackPosition=t.track_position,this.diskNumber=t.disk_number,this.releaseDate=new Date(t.release_date),this.bpm=t.bpm,this.gain=t.gain,this.contributors=[],t.contributors.forEach(r=>{this.contributors?.push(new M(r))}),this}toJSON(){return{id:this.id,title:this.title,shortTitle:this.shortTitle,url:this.url,durationInSec:this.durationInSec,rank:this.rank,explicit:this.explicit,previewURL:this.previewURL,artist:this.artist,album:this.album,type:this.type,trackPosition:this.trackPosition,diskNumber:this.diskNumber,releaseDate:this.releaseDate,bpm:this.bpm,gain:this.gain,contributors:this.contributors}}};a(g,"DeezerTrack");var q=class{constructor(e,t){if(this.id=e.id,this.title=e.title,this.url=e.link,this.recordType=e.record_type,this.explicit=e.explicit_lyrics,this.artist=new M(e.artist),this.type="album",this.tracksCount=e.nb_tracks,this.contributors=[],this.genres=[],this.tracks=[],this.cover={xl:e.cover_xl,big:e.cover_big,medium:e.cover_medium,small:e.cover_small},this.partial=t,!t){this.upc=e.upc,this.durationInSec=e.duration,this.numberOfFans=e.fans,this.releaseDate=new Date(e.release_date),this.available=e.available,e.contributors.forEach(s=>{this.contributors?.push(new M(s))}),e.genres.data.forEach(s=>{this.genres?.push({name:s.name,picture:{xl:`${s.picture}?size=xl`,big:`${s.picture}?size=big`,medium:`${s.picture}?size=medium`,small:`${s.picture}?size=small`}})});let r={id:this.id,title:this.title,cover_xl:this.cover.xl,cover_big:this.cover.big,cover_medium:this.cover.medium,cover_small:this.cover.small,release_date:e.release_date};e.tracks.data.forEach(s=>{s.album=r,this.tracks.push(new g(s,!0))})}}async fetch(){if(!this.partial)return this;let e=await h(`https://api.deezer.com/album/${this.id}/`).catch(s=>s);if(e instanceof Error)throw e;let t=JSON.parse(e);this.partial=!1,this.upc=t.upc,this.durationInSec=t.duration,this.numberOfFans=t.fans,this.releaseDate=new Date(t.release_date),this.available=t.available,this.contributors=[],this.genres=[],this.tracks=[],t.contributors.forEach(s=>{this.contributors?.push(new M(s))}),t.genres.data.forEach(s=>{this.genres?.push({name:s.name,picture:{xl:`${s.picture}?size=xl`,big:`${s.picture}?size=big`,medium:`${s.picture}?size=medium`,small:`${s.picture}?size=small`}})});let r={id:this.id,title:this.title,cover_xl:this.cover.xl,cover_big:this.cover.big,cover_medium:this.cover.medium,cover_small:this.cover.small,release_date:t.release_date};return t.tracks.data.forEach(s=>{s.album=r,this.tracks.push(new g(s,!0))}),this}async all_tracks(){return await this.fetch(),this.tracks}toJSON(){return{id:this.id,title:this.title,url:this.url,recordType:this.recordType,explicit:this.explicit,artist:this.artist,cover:this.cover,type:this.type,upc:this.upc,tracksCount:this.tracksCount,durationInSec:this.durationInSec,numberOfFans:this.numberOfFans,releaseDate:this.releaseDate,available:this.available,genres:this.genres,contributors:this.contributors,tracks:this.tracks.map(e=>e.toJSON())}}};a(q,"DeezerAlbum");var U=class{constructor(e,t){this.id=e.id,this.title=e.title,this.public=e.public,this.url=e.link,this.creationDate=new Date(e.creation_date),this.type="playlist",this.tracksCount=e.nb_tracks,this.tracks=[],this.picture={xl:e.picture_xl,big:e.picture_big,medium:e.picture_medium,small:e.picture_small},e.user?this.creator={id:e.user.id,name:e.user.name}:this.creator={id:e.creator.id,name:e.creator.name},this.partial=t,t||(this.description=e.description,this.durationInSec=e.duration,this.isLoved=e.is_loved_track,this.collaborative=e.collaborative,this.fans=e.fans,this.public&&(this.tracks=e.tracks.data.map(r=>new g(r,!0))))}async fetch(){if(!this.partial&&(this.tracks.length===this.tracksCount||!this.public))return this;if(this.partial){let t=await h(`https://api.deezer.com/playlist/${this.id}/`).catch(s=>s);if(t instanceof Error)throw t;let r=JSON.parse(t);this.partial=!1,this.description=r.description,this.durationInSec=r.duration,this.isLoved=r.is_loved_track,this.collaborative=r.collaborative,this.fans=r.fans,this.public&&(this.tracks=r.tracks.data.map(s=>new g(s,!0)))}let e=this.tracks.length;if(this.public&&e!==this.tracksCount){let t=this.tracksCount-e;t>1e3&&(t=1e3);let r=[];for(let o=1;o<=Math.ceil(t/100);o++)r.push(new Promise(async(l,c)=>{let u=await h(`https://api.deezer.com/playlist/${this.id}/tracks?limit=100&index=${o*100}`).catch(f=>c(f));if(typeof u!="string")return;let y=JSON.parse(u).data.map(f=>new g(f,!0));l(y)}));let s=await Promise.allSettled(r),n=[];for(let o of s)if(o.status==="fulfilled")n.push(...o.value);else throw o.reason;this.tracks.push(...n)}return this}async all_tracks(){return await this.fetch(),this.tracks}toJSON(){return{id:this.id,title:this.title,public:this.public,url:this.url,picture:this.picture,creationDate:this.creationDate,type:this.type,creator:this.creator,tracksCount:this.tracksCount,description:this.description,durationInSec:this.durationInSec,isLoved:this.isLoved,collaborative:this.collaborative,fans:this.fans,tracks:this.tracks.map(e=>e.toJSON())}}};a(U,"DeezerPlaylist");var Pe=class{constructor(e){this.id=e.id,this.title=e.title,this.url=`https://www.deezer.com/album/${e.id}/`,this.cover={xl:e.cover_xl,big:e.cover_big,medium:e.cover_medium,small:e.cover_small},e.release_date&&(this.releaseDate=new Date(e.release_date))}};a(Pe,"DeezerTrackAlbum");var M=class{constructor(e){this.id=e.id,this.name=e.name,this.url=e.link?e.link:`https://www.deezer.com/artist/${e.id}/`,e.picture_xl&&(this.picture={xl:e.picture_xl,big:e.picture_big,medium:e.picture_medium,small:e.picture_small}),e.role&&(this.role=e.role)}};a(M,"DeezerArtist");async function Ae(i){let e;try{e=new It.URL(i)}catch{return{type:"search"}}if(e.protocol!=="https:"&&e.protocol!=="http:")return{type:"search"};let t=e.pathname;t.endsWith("/")&&(t=t.slice(0,-1));let r=t.split("/");switch(e.hostname){case"deezer.com":case"www.deezer.com":{if(r.length===4){if(!r.splice(1,1)[0].match(/^[a-z]{2}$/))return{type:!1}}else if(r.length!==3)return{type:!1};return(r[1]==="track"||r[1]==="album"||r[1]==="playlist")&&r[2].match(/^\d+$/)?{type:r[1],id:r[2]}:{type:!1}}case"api.deezer.com":return r.length===3&&(r[1]==="track"||r[1]==="album"||r[1]==="playlist")&&r[2].match(/^\d+$/)?{type:r[1],id:r[2]}:{type:!1};case"deezer.page.link":if(r.length===2&&r[1].match(/^[A-Za-z0-9]+$/)){let s=await ie(i).catch(n=>n);return s instanceof Error?{type:!1,error:s.message}:await Ae(s)}else return{type:!1};default:return{type:"search"}}}a(Ae,"internalValidate");async function Ne(i){let e=await Ae(i.trim());if(e.error)throw new Error(`This is not a Deezer track, playlist or album URL:
${e.error}`);if(!e.type||e.type==="search")throw new Error("This is not a Deezer track, playlist or album URL");let t=await h(`https://api.deezer.com/${e.type}/${e.id}`).catch(s=>s);if(t instanceof Error)throw t;let r=JSON.parse(t);if(r.error)throw new Error(`Deezer API Error: ${r.error.type}: ${r.error.message}`);switch(e.type){case"track":return new g(r,!1);case"playlist":return new U(r,!1);case"album":return new q(r,!1)}}a(Ne,"deezer");async function Se(i){return(await Ae(i.trim())).type}a(Se,"dz_validate");async function Rt(i,e){let t=i.trim(),r=e.type??"track",s=e.limit??10,n=e.fuzzy??!0;if(t.length===0)throw new Error("A query is required to search.");if(s>100)throw new Error("The maximum search limit for Deezer is 100");if(s<1)throw new Error("The minimum search limit for Deezer is 1");if(r!=="track"&&r!=="album"&&r!="playlist")throw new Error(`"${r}" is not a valid Deezer search type`);t=encodeURIComponent(t);let o=await h(`https://api.deezer.com/search/${r}/?q=${t}&limit=${s}${n?"":"strict=on"}`).catch(u=>u);if(o instanceof Error)throw o;let l=JSON.parse(o);if(l.error)throw new Error(`Deezer API Error: ${l.error.type}: ${l.error.message}`);let c=[];switch(r){case"track":c=l.data.map(u=>new g(u,!0));break;case"playlist":c=l.data.map(u=>new U(u,!0));break;case"album":c=l.data.map(u=>new q(u,!0));break}return c}a(Rt,"dz_search");async function ze(i){let e=i.limit??10;if(e>100)throw new Error("The maximum search limit for Deezer is 100");if(e<1)throw new Error("The minimum search limit for Deezer is 1");let t=[];if(i.artist&&t.push(`artist:"${encodeURIComponent(i.artist.trim())}"`),i.album&&t.push(`album:"${encodeURIComponent(i.album.trim())}"`),i.title&&t.push(`track:"${encodeURIComponent(i.title.trim())}"`),i.label&&t.push(`label:"${encodeURIComponent(i.label.trim())}"`),isNaN(Number(i.minDurationInSec))||t.push(`dur_min:${i.minDurationInSec}`),isNaN(Number(i.maxDurationInSec))||t.push(`dur_max:${i.maxDurationInSec}`),isNaN(Number(i.minBPM))||t.push(`bpm_min:${i.minBPM}`),isNaN(Number(i.maxBPM))||t.push(`bpm_max:${i.maxBPM}`),t.length===0)throw new Error("At least one type of metadata is required.");let r=await h(`https://api.deezer.com/search/track/?q=${t.join(" ")}&limit=${e}`).catch(o=>o);if(r instanceof Error)throw r;let s=JSON.parse(r);if(s.error)throw new Error(`Deezer API Error: ${s.error.type}: ${s.error.message}`);return s.data.map(o=>new g(o,!0))}a(ze,"dz_advanced_track_search");async function Le(i){i.spotify&&await wt(i.spotify),i.soundcloud&&Dt(i.soundcloud),i.youtube&&Fe(i.youtube),i.useragent&&Ke(i.useragent)}a(Le,"setToken");var Ct=require("readline"),I=require("fs");async function Ot(i,e={}){let t=i.trim();if(t.length===0)throw new Error("Stream URL has a length of 0. Check your url again.");if(e.htmldata)return await ye(t,e);if(t.indexOf("spotify")!==-1)throw new Error("Streaming from Spotify is not supported. Please use search() to find a similar track on YouTube or SoundCloud instead.");if(t.indexOf("deezer")!==-1)throw new Error("Streaming from Deezer is not supported. Please use search() to find a similar track on YouTube or SoundCloud instead.");return t.indexOf("soundcloud")!==-1?await St(t,e.quality):await ye(t,e)}a(Ot,"stream");async function $t(i,e={}){e.source||(e.source={youtube:"video"});let t=encodeURIComponent(i.trim());if(e.source.youtube)return await ft(t,{limit:e.limit,type:e.source.youtube,language:e.language,unblurNSFWThumbnails:e.unblurNSFWThumbnails});if(e.source.spotify)return await gt(t,e.source.spotify,e.limit);if(e.source.soundcloud)return await vt(t,e.source.soundcloud,e.limit);if(e.source.deezer)return await Rt(t,{limit:e.limit,type:e.source.deezer,fuzzy:e.fuzzy});throw new Error("Not possible to reach Here LOL. Easter Egg of play-dl if someone get this.")}a($t,"search");async function Pt(i,e={}){return i instanceof T?await Et(i,e.quality):await be(i,e)}a(Pt,"stream_from_info");async function At(i){let e,t=i.trim();return t.startsWith("https")?t.indexOf("spotify")!==-1?(e=ge(t),e!==!1?"sp_"+e:!1):t.indexOf("soundcloud")!==-1?(e=await ve(t),e!==!1?"so_"+e:!1):t.indexOf("deezer")!==-1?(e=await Se(t),e!==!1?"dz_"+e:!1):(e=Y(t),e!==!1?"yt_"+e:!1):"search"}a(At,"validate");function Nt(){let i=(0,Ct.createInterface)({input:process.stdin,output:process.stdout});i.question("Do you want to save data in a file ? (Yes / No): ",e=>{let t;if(e.toLowerCase()==="yes")t=!0;else if(e.toLowerCase()==="no")t=!1;else{console.log("That option doesn't exist. Try again..."),i.close();return}i.question("Choose your service - sc (for SoundCloud) / sp (for Spotify)  / yo (for YouTube): ",r=>{if(r.toLowerCase().startsWith("sp")){let s,n,o,l;i.question("Start by entering your Client ID : ",c=>{s=c,i.question("Now enter your Client Secret : ",u=>{n=u,i.question("Enter your Redirect URL now : ",m=>{o=m,console.log(`
If you would like to know your region code visit : 
https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2#Officially_assigned_code_elements 
`),i.question("Enter your region code (2-letter country code) : ",y=>{y.length===2?l=y:(console.log("That doesn't look like a valid region code, IN will be selected as default."),l="IN"),console.log(`
...
`),i.question("Paste the url which you just copied : ",async f=>{(0,I.existsSync)(".data")||(0,I.mkdirSync)(".data");let b={client_id:s,client_secret:n,redirect_url:o,authorization_code:f.split("code=")[1],market:l};if(await bt(b,t)===!1)throw new Error("Failed to get access token.");i.close()})})})})})}else if(r.toLowerCase().startsWith("sc")){if(!t){console.log("You already had a client ID, just paste that in setToken function."),i.close();return}i.question("Client ID : ",async s=>{let n=s;if(!n){console.log("You didn't provide a client ID. Try again..."),i.close();return}(0,I.existsSync)(".data")||(0,I.mkdirSync)(".data"),console.log("Validating your client ID, hold on..."),await Tt(n)?(console.log("Client ID has been validated successfully."),(0,I.writeFileSync)(".data/soundcloud.data",JSON.stringify({client_id:n},void 0,4))):console.log("That doesn't look like a valid client ID. Retry with a correct client ID."),i.close()})}else if(r.toLowerCase().startsWith("yo")){if(!t){console.log("You already had cookie, just paste that in setToken function."),i.close();return}i.question("Cookies : ",s=>{if(!s||s.length===0){console.log("You didn't provide a cookie. Try again..."),i.close();return}(0,I.existsSync)(".data")||(0,I.mkdirSync)(".data"),console.log("Cookies has been added successfully.");let n={};s.split(";").forEach(o=>{let l=o.split("=");if(l.length<=1)return;let c=l.shift()?.trim(),u=l.join("=").trim();Object.assign(n,{[c]:u})}),(0,I.writeFileSync)(".data/youtube.data",JSON.stringify({cookie:n},void 0,4)),i.close()})}else console.log("That option doesn't exist. Try again..."),i.close()})})}a(Nt,"authorization");function zt(i,e){let t=i.listeners("idle");for(let o of t)o.__playDlAttachedListener&&(o(),i.removeListener("idle",o));let r=a(()=>e.pause(),"pauseListener"),s=a(()=>e.resume(),"resumeListener"),n=a(()=>{i.removeListener("paused",r),i.removeListener("autopaused",r),i.removeListener("playing",s)},"idleListener");r.__playDlAttachedListener=!0,s.__playDlAttachedListener=!0,n.__playDlAttachedListener=!0,i.on("paused",r),i.on("autopaused",r),i.on("playing",s),i.once("idle",n)}a(zt,"attachListeners");var Si={DeezerAlbum:q,DeezerPlaylist:U,DeezerTrack:g,SoundCloudPlaylist:P,SoundCloudStream:L,SoundCloudTrack:T,SpotifyAlbum:z,SpotifyPlaylist:N,SpotifyTrack:E,YouTubeChannel:v,YouTubePlayList:D,YouTubeVideo:S,attachListeners:zt,authorization:Nt,decipher_info:j,deezer:Ne,dz_advanced_track_search:ze,dz_validate:Se,extractID:ue,getFreeClientID:$e,is_expired:Oe,playlist_info:he,refreshToken:we,search:$t,setToken:Le,so_validate:ve,soundcloud:ke,spotify:Ce,sp_validate:ge,stream:Ot,stream_from_info:Pt,validate:At,video_basic_info:H,video_info:ce,yt_validate:Y};0&&(module.exports={DeezerAlbum,DeezerPlaylist,DeezerTrack,SoundCloudPlaylist,SoundCloudStream,SoundCloudTrack,SpotifyAlbum,SpotifyPlaylist,SpotifyTrack,YouTubeChannel,YouTubePlayList,YouTubeVideo,attachListeners,authorization,decipher_info,deezer,dz_advanced_track_search,dz_validate,extractID,getFreeClientID,is_expired,playlist_info,refreshToken,search,setToken,so_validate,soundcloud,sp_validate,spotify,stream,stream_from_info,validate,video_basic_info,video_info,yt_validate});
