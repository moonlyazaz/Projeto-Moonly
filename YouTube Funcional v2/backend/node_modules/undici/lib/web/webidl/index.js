...
webidl.util.ConvertToInt = function (V, bitLength, signedness, flags) {
  ...
  if (bitLength === 64) {
    // 1. Let upperBound be 2^53 ÃÂ¢ÃÂÃÂ 1.
    ...
    if (signedness === 'unsigned') {
      ...
      // 3. Otherwise let lowerBound be ÃÂ¢ÃÂÃÂ2^53 + 1.
    ...
    // 2. Let upperBound be 2^bitLength ÃÂ¢ÃÂÃÂ 1.
    upperBound = Math.pow(2, bitLength) - 1
  } else {
    // 3. Otherwise:

    // 1. Let lowerBound be -2^(bitLength ÃÂ¢ÃÂÃÂ 1).
    lowerBound = -Math.pow(2, bitLength - 1)

    // 2. Let upperBound be 2^(bitLength ÃÂ¢ÃÂÃÂ 1) ÃÂ¢ÃÂÃÂ 1.
  ...
  // 5. If x is ÃÂ¢ÃÂÃÂ0, then set x to +0.
  if (x === 0) {
    x = 0
  }
  ...
  if (webidl.util.HasFlag(flags, webidl.attributes.EnforceRange)) {
    // 1. If x is NaN, +ÃÂ¢ÃÂÃÂ, or ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ, then throw a TypeError.
    if (
      Number.isNaN(x) ||
  ...
  if (!Number.isNaN(x) && webidl.util.HasFlag(flags, webidl.attributes.Clamp)) {
    ...
    //    and choosing +0 rather than ÃÂ¢ÃÂÃÂ0.
    if (Math.floor(x) % 2 === 0) {
      x = Math.floor(x)
    } else {
      x = Math.ceil(x)
  ...
  // 8. If x is NaN, +0, +ÃÂ¢ÃÂÃÂ, or ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ, then return +0.
  if (
    Number.isNaN(x) ||
    (x === 0 && Object.is(0, x)) ||
    x === Number.POSITIVE_INFINITY ||
  ...
  // 11. If signedness is "signed" and x ÃÂ¢ÃÂÃÂ¥ 2^(bitLength ÃÂ¢ÃÂÃÂ 1),
  //    then return x ÃÂ¢ÃÂÃÂ 2^bitLength.
...
webidl.util.IntegerPart = function (n) {
  ...
  // 2. If n < 0, then return -1 ÃÂÃÂ r.
  if (n < 0) {
    return -1 * r
  }
...
webidl.util.getCopyOfBytesHeldByBufferSource = function (bufferSource) {
  ...
  // 9. For i in the range offset to offset + length ÃÂ¢ÃÂÃÂ 1, inclusive,
  //    set bytes[i ÃÂ¢ÃÂÃÂ offset] to GetValueFromBuffer(jsArrayBuffer, i, Uint8, true, Unordered).
...
webidl.converters.TypedArray = function (V, T, prefix, argument, flags) {
  ...
  //    equal to TÃÂ¢ÃÂÃÂs name, then throw a TypeError.
  if (
    webidl.util.Type(V) !== OBJECT ||
