 ...
 *
 * The valid SRI hash algorithm token set is the ordered set ÃÂÃÂ« "sha256",
 * "sha384", "sha512" ÃÂÃÂ» (corresponding to SHA-256, SHA-384, and SHA-512
...
function getStrongestMetadata (metadataList) {
  ...
  for (const item of metadataList) {
    ...
    // currentAlgorithmIndex:
      ...
      // 2. Set result to ÃÂÃÂ« item ÃÂÃÂ».
      result[0] = item
      result.length = 1
...
function parseMetadata (metadata) {
  ...
  for (const item of metadata.split(' ')) {
    ...
    // ÃÂÃÂ«["alg" ÃÂ¢ÃÂÃÂ algorithm, "val" ÃÂ¢ÃÂÃÂ base64-value]ÃÂÃÂ».
    const metadata = {
      alg: algorithm,
      val: base64Value
    }
