...
class WebSocketStream {
  ...
  constructor (url, options = undefined) {
    ...
    if (options.signal != null) {
      ...
      // 8.2. If signal is aborted , then reject this 's opened promise and closed promise with signal ÃÂ¢ÃÂÃÂs abort reason
      //      and return.
      if (signal.aborted) {
      ...
      signal.addEventListener('abort', () => {
        ...
        if (!isEstablished(this.#handler.readyState)) {
          ...
          // Reject this 's opened promise and closed promise with signal ÃÂ¢ÃÂÃÂs abort reason .
          this.#openedPromise.reject(signal.reason)
          this.#closedPromise.reject(signal.reason)
  ...
  #write (chunk) {
    ...
    // 1. Let promise be a new promise created in stream ÃÂ¢ÃÂÃÂs relevant realm .
    ...
    // 6.3. Queue a global task on the WebSocket task source given stream ÃÂ¢ÃÂÃÂs relevant global object to resolve promise with undefined.
    return promise.promise
  }
  ...
  #onConnectionEstablished (response, parsedExtensions) {
    ...
    // 1. Change stream ÃÂ¢ÃÂÃÂs ready state to OPEN (1).
    this.#handler.readyState = states.OPEN

    // 2. Set stream ÃÂ¢ÃÂÃÂs was ever connected to true.
    ...
    // Set stream ÃÂ¢ÃÂÃÂs readable stream to readable .
    this.#readableStream = readable

    // Resolve stream ÃÂ¢ÃÂÃÂs opened promise with WebSocketOpenInfo ÃÂÃÂ«[ " extensions " ÃÂ¢ÃÂÃÂ extensions , " protocol " ÃÂ¢ÃÂÃÂ protocol , " readable " ÃÂ¢ÃÂÃÂ readable , " writable " ÃÂ¢ÃÂÃÂ writable ]ÃÂÃÂ».
  ...
  #onMessage (type, data) {
    // 1. If streamÃÂ¢ÃÂÃÂs ready state is not OPEN (1), then return.
    ...
    // 3. Enqueue chunk into streamÃÂ¢ÃÂÃÂs readable stream.
  ...
  #onSocketClose () {
    ...
    // 2. If stream ÃÂ¢ÃÂÃÂs handshake aborted is true, then return.
    if (this.#handshakeAborted) {
      return
    }

    // 3. If stream ÃÂ¢ÃÂÃÂs was ever connected is false, then reject stream ÃÂ¢ÃÂÃÂs opened promise with a new WebSocketError.
    ...
    if (wasClean) {
      // 6.1. Close stream ÃÂ¢ÃÂÃÂs readable stream .
      this.#readableStreamController.close()

      // 6.2. Error stream ÃÂ¢ÃÂÃÂs writable stream with an " InvalidStateError " DOMException indicating that a closed WebSocketStream cannot be written to.
      this.#writableStreamController.error(new DOMException('A closed WebSocketStream cannot be written to', 'InvalidStateError'))

      // 6.3. Resolve stream ÃÂ¢ÃÂÃÂs closed promise with WebSocketCloseInfo ÃÂÃÂ«[ " closeCode " ÃÂ¢ÃÂÃÂ code , " reason " ÃÂ¢ÃÂÃÂ reason ]ÃÂÃÂ».
      ...
      // 7.2. Error stream ÃÂ¢ÃÂÃÂs readable stream with error .
      this.#readableStreamController?.error(error)

      // 7.3. Error stream ÃÂ¢ÃÂÃÂs writable stream with error .
      this.#writableStreamController?.error(error)

      // 7.4. Reject stream ÃÂ¢ÃÂÃÂs closed promise with error .
  ...
  #closeUsingReason (reason) {
    ...
    if (webidl.is.WebSocketError(reason)) {
      // 3.1. Set code to reason ÃÂ¢ÃÂÃÂs closeCode .
      code = reason.closeCode

      // 3.2. Set reasonString to reason ÃÂ¢ÃÂÃÂs reason .
